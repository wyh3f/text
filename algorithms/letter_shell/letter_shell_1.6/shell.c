...
const SHELL_CommandTypeDef shellCommandList[] = 
{
    ...
    {(uint8_t *)"paraTest", (void (*)())shellParaTest, (uint8_t *)"test parameter"},
		{(uint8_t *)"show",    (void (*)())showMyVar,   (uint8_t *)"show my variable"},
		{(uint8_t *)"set",     (void (*)())setMyVar,    (uint8_t *)"set my variable <value>"},
...
void shellInit(void)
{
    ...
    for (int i = 0; i < SHELL_PARAMETER_MAX_NUMBER; i++)
    {
		...
		//开启串口接收中断
		HAL_UART_Receive_IT(&shellUart, &usart1RxData, 1);
...
*           任务处理
*******************************************************************************/
void shellHandler(UART_HandleTypeDef *huart)
{
    static uint8_t runFlag;
    static CONTROL_Status controlFlag = CONTROL_FREE;
		uint8_t receiveData = usart1RxData;
	
	
		if (huart != &shellUart) return ;
    ...
    switch (receiveData)
    {
        ...
        case '\n':
            ...
            else
            {
                ...
                for (int32_t i = sizeof(shellCommandList) / sizeof(SHELL_CommandTypeDef) - 1;i >=  0; i--)
                {
                    if (strcmp((const char *)commandPara[0],(const char *)shellCommandList[i].name) == 0)
                    {
                        runFlag = 1;
                ...
                shellDisplay("\r\n");
                runFlag = 0;
                for (int32_t i = sizeof(shellCommandList) / sizeof(SHELL_CommandTypeDef) - 1;
                     i >=  0; i--)
                {
...
void Shell_Printf(char *format, ...) 
{
	...
	HAL_UART_Transmit(&shellUart, (uint8_t*)formatBuf, len, HAL_MAX_DELAY);    
}
...
{
    // 检查是否有且仅有一个实际参数
    if (argc != 2) {   // argc=2 表示命令名 + 1个参数
        Shell_Printf("Usage: set <value>\r\n");
        return 0;
    }
    // 转换第二个参数（argv[1]）为数值
    myCounter = atoi((char*)argv[1]);
    Shell_Printf("myCounter set to %d\r\n", myCounter);
    return 0;
}
